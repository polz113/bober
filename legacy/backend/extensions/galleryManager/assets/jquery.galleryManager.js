(function ($) {
    ...
    function galleryManager(el, options) {
        ...
        if (!jQuery.GalleryImages) {
            jQuery.GalleryImages = {};
        }
        jQuery.GalleryImages[opts.wId] = $images;
        if (!jQuery.GalleryEditorForm) {
            jQuery.GalleryEditorForm = {};
        }
        jQuery.GalleryEditorForm[opts.wId] = $editorForm;
        
        if (!jQuery.GalleryPhotoTemplate) {
            jQuery.GalleryPhotoTemplate = {};
        }
        jQuery.GalleryPhotoTemplate[opts.wId] = photoTemplate;
        
        if (!jQuery.GalleryBindPhotoEvents) {
            jQuery.GalleryBindPhotoEvents = {};
        }
        jQuery.GalleryBindPhotoEvents[opts.wId] = bindPhotoEvents;
        ...
        function photoTemplate(id, src, name, description, rank) {
            ...
            res += '</div><input type="hidden" name="order[' + id + ']" value="' + rank + '"/><div class="actions">' +
                ...
                '</div><input type="checkbox" class="photo-select"/>'+
                '<a href="'+src.replace('/_', '/')+'" class="lightbox" target="_blank" style="padding-left: 75px; position: relative;"><img style="padding-top: 5px; width: 16px; height: 16px;" src="/images/zoom_in.png" width="16" height="16" alt="+" /></a>'+
                '</div>';
            return res;
        }
