...
class QuestionController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow authenticated user to perform all available actions
                'actions' => array('index', 'view', 'create', 'update', 'admin', 'delete', 'activate', 'deactivate', 'import'),
                'users' => array('@'),
            ),
    ...
    public function actionImport() {
        if ($this->CanAccess('import')) {
            ...
            if (isset($_POST['Question'])) {
                $model->attributes = $_POST['Question'];
                if (isset($_FILES['Question']['tmp_name']['uploadedData']) && file_exists($_FILES['Question']['tmp_name']['uploadedData'])) {
                    $id = $model->processImport($_FILES['Question']['tmp_name']['uploadedData']);
                    if ($id > 0) {
                        $this->redirect(array('view', 'id' => $id));
                    }else{
                        throw new CHttpException(405, Yii::t('app', 'Save question failed.'));
                    }
                } else {
                    throw new CHttpException(405, Yii::t('app', 'File does not define'));
                }
